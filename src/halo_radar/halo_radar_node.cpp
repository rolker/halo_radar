...
public:
    RosRadar(halo_radar::AddressSet const &addresses):halo_radar::Radar(addresses)
    {
        ...
        m_state_change_sub = n.subscribe("radar/"+addresses.label+"/change_state",10,&RosRadar::stateChangeCallback,this);
...
private:
    ...
    void createEnumControl(std::string const &name, std::string const &label, std::string const enums[], marine_msgs::RadarControlSet &rcs)
    {
    ...
    void createFloatControl(std::string const &name, std::string const &label, float min_value, float max_value, marine_msgs::RadarControlSet &rcs)
    {
        if(m_state.find(name) != m_state.end())
        {
            marine_msgs::RadarControlItem rci;
    ...
    void createFloatWithAutoControl(std::string const &name, std::string const & auto_name, std::string const &label, float min_value, float max_value, marine_msgs::RadarControlSet &rcs)
    {
